 ...
 *      ip归属地 单机版
 */
object IpLocationSingle {

  def main(args: Array[String]): Unit = {
    val conf: SparkConf = new SparkConf().setAppName("IpLocationSingle").setMaster("local[4]")
    ...
    val func = (line: String) => {
      ...
      //（该函数中的代码是在Executor中分别调用执行的，通过广播变量的引用，就可以拿到当前Executor中的广播的规则了）
