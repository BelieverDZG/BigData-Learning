...
object CustomSortFour {
  ...
  def main(args: Array[String]): Unit = {
    val conf: SparkConf = new SparkConf().setAppName("CustomSortFour").setMaster("local[4]")
    ...
    //将RDD里面的封装类型的数据进行排序
    import SortRules.OrderingFreshMan
    val sorted: RDD[(String, Int, Int)] = tpRDD.sortBy(tp => FreshMan(tp._2, tp._3))
...
case class FreshMan(age: Int, fv: Int)
